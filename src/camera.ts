...
class Camera extends Element {
    ...
    add() {
        ...
        // picker (WebGPU rejects 0x0 textures; use at least 1x1)
        const { width, height } = this.scene.targetSize;
        this.picker = new Picker(this.scene.app, Math.max(1, width), Math.max(1, height));
    ...
    pickRect(x: number, y: number, width: number, height: number) {
        const device = this.scene.graphicsDevice as WebglGraphicsDevice;
        // WebGPU has no readPixels in the same form; return no hits
        if (device.isWebGPU || typeof device.readPixels !== 'function') {
            return new Array(width * height).fill(-1);
        }

        const pixels = new Uint8Array(width * height * 4);

        // read pixels (updateBegin/updateEnd are WebGL-only)
        // @ts-ignore
        device.setRenderTarget(this.picker.renderTarget);
        if (typeof device.updateBegin === 'function') {
            device.updateBegin();
        }
        // @ts-ignore
        device.readPixels(x, this.picker.renderTarget.height - y - height, width, height, pixels);
        if (typeof device.updateEnd === 'function') {
            device.updateEnd();
        }
