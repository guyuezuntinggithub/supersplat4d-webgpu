...
class DataProcessor {
    ...
    // calculate the intersection of a mask canvas with splat centers (WebGPU readPixels differs)
    intersect(options: MaskOptions | RectOptions | SphereOptions | BoxOptions, splat: Splat) {
        const { device } = this;
        if (device.isWebGPU || typeof (device as WebglGraphicsDevice).readPixels !== 'function') {
    ...
    // all visible splats (WebGPU has no .gl.readPixels; fallback: boundingBox keeps resource AABB)
    calcBound(splat: Splat, boundingBox: BoundingBox, onlySelected: boolean) {
        ...
        if (typeof glDevice.updateBegin === 'function') {
        ...
        if (typeof glDevice.updateEnd === 'function') {
            glDevice.updateEnd();
        }
    ...
    // calculate world-space splat positions (WebGPU has no .gl.readPixels)
